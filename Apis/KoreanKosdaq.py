...
module_logger = logging.getLogger('Apis.KoreanKosdaq')
module_logger.setLevel(logging.INFO)
module_logger.addHandler(ch)

class KoreanKosdaqInfo(AbsTROccurs):
    def __init__(self):
...
class KoreanKosdaqHoga(AbsReal):  ## 해외 선물 호가
    def __init__(self):
...
class KoreanKosdaqChegyul(AbsReal):  ## 해외선물 체결
...
class KoreanKosdaqCounter(AbsReal):  ## 해외선물 체결
    ...
    def _open_file(self):  # I/O
        AbsReal._open_file(self, "KOREANSTOCK", "KOREANSTOCK_KOSDAQ_COUNTER")

    def start(self):
        AbsReal.start(self, self.code)

def getKoreanKosdaqInfo():
    info_handler = KoreanKosdaqInfo.getInstance()
    info_handler.start()
    info_handler.singleRequest(2) # 0 :전체 1: 코스피 2: 코스닥

    while info_handler.is_data_received == False:
        pythoncom.PumpWaitingMessages()

    info_handler.saveResultData("KOREANSTOCK", "KOREANSTOCK_KOSDAQ_INFO") # dataframe csv file save
    df_data =  info_handler.getResultData()

    module_logger.info("Get korean stock(KOSDAQ) info successfully")

    return df_data


def getKoreanKosdaqRealData(list_shcode):
    hoga_handler = KoreanKosdaqHoga.getInstance()
    hoga_handler.start()

    chegyul_handler = KoreanKosdaqChegyul.getInstance()
    chegyul_handler.start()

    counter_handler = KoreanKosdaqCounter.getInstance()
    ...
    module_logger.info("Added korean stock(KOSDAQ) codes well :: total codes are " + str(nums) + ".")
